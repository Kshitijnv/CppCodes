...
{
    ofstream file;
    file.open("Account_list.txt", ios::app | ios::binary);
    if (!file)
    {
    ...
    file << "Account Name: " << Account_name << endl;
    file << "Account Type: " << Account_type << endl;
    file << "Account Balance: " << Account_bal << endl;
    file << "-------------------" << endl;
    file.close();
}
...
int main()
{
    ifstream infile("Account_list.txt");        //loading the file when starting the program into infile OBJECT
